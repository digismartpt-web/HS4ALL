...
window.updateSimpleSoundManager = function (soundId, src) {
    ...
    if (window.simpleSoundManager.sounds[soundId]) {
        ...
        // Only update if source changed to avoid unnecessary reloads
        if (audio.src !== src) {
            audio.src = src;
            audio.load();
            console.log(`Updated existing sound '${soundId}' with new source.`);
        }
    } else {
        ...
        console.log(`Created new sound '${soundId}' with custom source.`);
    }
