...
document.addEventListener('DOMContentLoaded', () => {
    ...
    // SECONDARY INITIALIZATION: Defer to allow hero to render first
    // Using setTimeout(0) to push to the end of the task queue
    setTimeout(() => {
        try {
            initScrollReveal();
            initParallax();
...
/**
 * Logo Image Initialization
 */
async function initLogoImage() {
    try {
        ...
        if (logoImgData) {
            if (navImg) {
                navImg.src = logoImgData;
                navImg.style.display = 'block';
            }
            if (navText) navText.style.display = 'none';

            if (footerImg) {
                footerImg.src = logoImgData;
                    ...
                    if (customTexts[lang] && window.translations[lang]) {
                        Object.keys(customTexts[lang]).forEach(key => {
                            ...
                            if (val !== null && val !== undefined && val !== '') {
                                // If the value contains SVG or translation artifacts (corrupted API response like <g id="1"> or <ex id="2" />),
...
function setLanguage(lang) {
    ...
    elements.forEach(el => {
        ...
        if (window.translations[lang][key]) {
            ...
            if (el.tagName === 'H1' || el.tagName === 'H2' || el.tagName === 'ADDRESS' || el.classList.contains('section-title')) {
                el.innerHTML = val;
            ...
            // Sync href for contact links
            if (el.tagName === 'A') {
                if (key === 'contact.email') el.href = `mailto:${val}`;
                if (key === 'contact.phone') el.href = `tel:${val.replace(/\s/g, '')}`;
            }
        }
    });
